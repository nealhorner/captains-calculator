...
let buildings = [...rawData.machines_and_buildings, ...rawDataMines.mines];
  ...
  items.forEach((m) => {
    ...
    m.recipes.forEach((r) => {
      try {
        ...
        if (RECIPES_DATA.hasOwnProperty(recipeId)) {
          let dupsCount =
            Object.keys(RECIPES_DATA).filter((r) => r.startsWith(recipeId + '_')).length + 1;
        ...
        r.inputs.forEach((p) => {
          ...
          if (PRODUCTS_DATA.hasOwnProperty(product.id)) {
            if (PRODUCTS_DATA[product.id].recipes.input.indexOf(recipeId) < 0) {
              PRODUCTS_DATA[product.id].recipes.input = [
                ...PRODUCTS_DATA[product.id].recipes.input,
                recipeId,
          ...
          if (PRODUCTS_DATA[product.id].machines.input.indexOf(machine.id) < 0) {
            PRODUCTS_DATA[product.id].machines.input = [
              ...PRODUCTS_DATA[product.id].machines.input,
              machine.id,
            ].sort((a, b) => a.localeCompare(b));
          }
          ...
          if (machine.products.input.indexOf(product.id) < 0) {
            machine.products.input = [...machine.products.input, product.id].sort((a, b) =>
              a.localeCompare(b),
            );
          }
        ...
        r.outputs.forEach((p) => {
          ...
          if (PRODUCTS_DATA.hasOwnProperty(product.id)) {
            if (PRODUCTS_DATA[product.id].recipes.output.indexOf(recipeId) < 0) {
              PRODUCTS_DATA[product.id].recipes.output = [
                ...PRODUCTS_DATA[product.id].recipes.output,
                recipeId,
              ].sort((a, b) => a.localeCompare(b));
            }
            ...
            if (PRODUCTS_DATA[product.id].machines.output.indexOf(machine.id) < 0) {
              PRODUCTS_DATA[product.id].machines.output = [
                ...PRODUCTS_DATA[product.id].machines.output,
                machine.id,
              ].sort((a, b) => a.localeCompare(b));
            }
          ...
          if (machine.products.output.indexOf(product.id) < 0) {
            machine.products.output = [...machine.products.output, product.id].sort((a, b) =>
              a.localeCompare(b),
            );
          }
    ...
    if (CATEGORIES_DATA.hasOwnProperty(category.id)) {
      CATEGORIES_DATA[category.id] = {
        ...CATEGORIES_DATA[category.id],
        machines: [...CATEGORIES_DATA[category.id].machines, machine.id].sort((a, b) =>
          a.localeCompare(b),
        ),
      };
      machine.recipes.forEach((r) => {
        if (CATEGORIES_DATA[category.id].recipes.indexOf(r) < 0) {
          CATEGORIES_DATA[category.id].recipes = [...CATEGORIES_DATA[category.id].recipes, r].sort(
