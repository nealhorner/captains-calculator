...
function FormDrawer<
  PayloadType extends GenericDictionary,
  ModelType extends GenericModel,
  IDType extends any,
>({
  formSchema,
  createAction,
  ...
  FormDrawerProps<PayloadType, ModelType, IDType>
>): React.ReactElement | null {
  ...
  const openDeleteModal = () =>
    modals.openConfirmModal({
      ...
      children: (
        <Text size="sm">
          Are you sure you want to delete this item? This action is destructive and cannot be
          undone.
        </Text>
      ),
    ...
    return (
      <Alert title="Error" variant="outline" m={7} color="red">
        {errors.map((error, key) => {
          return <span key={`error-${key}`}>{error.text}</span>;
        })}
      </Alert>
    );
  };
