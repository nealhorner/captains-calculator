...
import { Box, Card, Group, Image, NumberInput, Stack, Text, Tooltip, UnstyledButton } from '@mantine/core';
...
export const TargetListCard: React.FC<TargetListCardProps> = ({ target }) => {
    ...
    const handleRemove = () => {
        modals.openConfirmModal({
            ...
            children: (
                <Text size="sm">
                    Remove <strong>{product ? product.name : 'this target'}</strong> from your
                    production chain? Buildings only needed for this target will be removed too.
                </Text>
            ),
                ...
                {target.recipeId ? (
                    <React.Fragment>
                        <NumberInput
                            ...
                            precision={2}
                            noClampOnBlur
                            rightSection={<Text size="xs" color="dimmed" pr={4}>/60s</Text>}
                        ...
                        {node && (
                            <Text size="xs" color="dimmed">
                                ...
                                {node.buildingsRequired !== node.machinesCount && ` (build ${node.buildingsRequired})`}
                            </Text>
                        )}
                    </React.Fragment>
                ) : (
                    <Text size="xs" color="dimmed">Pick a building and recipe to finish this target.</Text>
                )}
