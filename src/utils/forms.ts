    ...
    if (field.validation && typeof field.validation === 'object') {
      ...
      Object.keys(field.validation).forEach((fieldValidationKey) => {
        ...
        if (fieldValidatorConfig) {
          let fieldValidatorObject = buildFieldValidator(
            fieldValidationKey as AvailableValidators,
...
const buildFieldValidator = <T extends keyof YupValidators>(
  fieldValidatorKey: AvailableValidators,
  fieldValidatorObject: YupValidators[T],
  field: FormSchemaField,
  existingFieldValidator: any,
  customValidators: CustomValidators,
) => {
    ...
    switch (fieldValidatorKey) {
      ...
      case 'type':
        ...
        if (typeFieldValidatorConfig.value === 'string') {
          let validationErrorMessage =
            parseValidationErrorMessage(typeFieldValidatorConfig.message, field.label) ||
      ...
      case 'required':
        ...
        if (requiredFieldValidatorConfig.value) {
          let validationErrorMessage =
            parseValidationErrorMessage(requiredFieldValidatorConfig.message, field.label) ||
            'Value is required';
      ...
      case 'domain_available':
        let domainValidator = customValidators.hasOwnProperty('domain_available')
          ? customValidators.domain_available
          : null;
        ...
        if (domainValidator) {
          if (existingFieldValidator !== null) {
            fieldValidator = existingFieldValidator.test(
              fieldValidatorKey,
            ...
            fieldValidator = yup
              .string()
              .test(fieldValidatorKey, domainValidationErrorMessage, domainValidator);
          }
        }
      ...
      case 'email_available':
        let emailValidator = customValidators.hasOwnProperty('email_available')
          ? customValidators.email_available
          : null;
        ...
        if (emailValidator) {
          if (existingFieldValidator !== null) {
            fieldValidator = existingFieldValidator.test(
              fieldValidatorKey,
              emailAvailableValidationErrorMessage,
              emailValidator,
            );
          } else {
            fieldValidator = yup
              .string()
              .test(fieldValidatorKey, emailAvailableValidationErrorMessage, emailValidator);
          }
        }
      ...
      case 'alphanumeric':
        let alphaNumValidationErrorMessage =
          'Invalid character, only letters, numbers, or hyphens allowed';

        if (existingFieldValidator !== null) {
          fieldValidator = existingFieldValidator.matches(
            /^[a-z0-9-]+$/i,
            alphaNumValidationErrorMessage,
          );
        } else {
...
const parseValidationErrorMessage = (
...
export const FORM_PHONE_VALIDATION_REGEX =
...
export const extractCurrentItemData = <T>(
  initialState: GenericDictionary,
  currentItem: GenericDictionary,
): T => {
