...
const node = ({ id, inputs = {}, outputs = {}, imports = {}, kind = 'normal', pinned = null }: NodeSpec): SolverNode => ({
    id,
    recipeInputs: inputs,
    recipeOutputs: outputs,
    imports: Object.keys(imports).reduce((acc, productId) => ({
        ...acc,
...
describe('solveChain', () => {
    ...
    it('sizes a single node from its target volume', () => {
        ...
        const { nodes: solved } = solveChain(nodes, [{ nodeId: 'smelter', productId: 'steel', quantity: 100 }])
    ...
    it('sizes an upstream supplier to meet derived demand', () => {
        ...
        const { nodes: solved } = solveChain(nodes, [{ nodeId: 'smelter', productId: 'steel', quantity: 90 }])
    ...
    it('sums demand on a merged supplier feeding two targets', () => {
        ...
        const nodes = [
            node({ id: 'a', inputs: { iron: 20 }, outputs: { plate: 10 }, imports: { iron: [{ source: 'foundry' }] } }),
            node({ id: 'b', inputs: { iron: 30 }, outputs: { beam: 10 }, imports: { iron: [{ source: 'foundry' }] } }),
    ...
    it('sizes a shared supplier from combined demand through a diamond', () => {
        ...
        const nodes = [
            ...
            node({ id: 'midX', inputs: { base: 5 }, outputs: { x: 10 }, imports: { base: [{ source: 'base' }] } }),
            node({ id: 'midY', inputs: { base: 7 }, outputs: { y: 10 }, imports: { base: [{ source: 'base' }] } }),
            node({ id: 'base', outputs: { base: 1 } })
        ]
        const { nodes: solved } = solveChain(nodes, [{ nodeId: 'root', productId: 'final', quantity: 10 }])
    ...
    it('splits input demand evenly across two suppliers of the same product', () => {
        ...
        const { nodes: solved } = solveChain(nodes, [{ nodeId: 'consumer', productId: 'plate', quantity: 50 }])
    ...
    it('sizes a multi-output recipe from the driving output and reports the other as surplus', () => {
        ...
        const { nodes: solved } = solveChain(nodes, [{ nodeId: 'separator', productId: 'oxygen', quantity: 72 }])
    ...
    it('treats a mine with a zero-quantity output as an infinite source', () => {
        ...
        const { nodes: solved } = solveChain(nodes, [{ nodeId: 'smelter', productId: 'iron', quantity: 60 }])
    ...
    it('passes demand straight through a storage node, ignoring capacity', () => {
        ...
        const { nodes: solved } = solveChain(nodes, [{ nodeId: 'consumer', productId: 'product', quantity: 25 }])
    ...
    it('reports an unsupplied input as a raw-input deficit', () => {
        const nodes = [node({ id: 'smelter', inputs: { iron: 30 }, outputs: { steel: 45 } })]
        const { nodes: solved } = solveChain(nodes, [{ nodeId: 'smelter', productId: 'steel', quantity: 45 }])
