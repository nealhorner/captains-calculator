...
export const setTargetRecipe: AsyncAction<RecipeId> = async ({ state, actions }, recipeId) => {
    ...
    let output = productId ? recipe.outputs.find((o: RecipeProduct) => o.id === productId) : undefined
...
export const setTargetQuantity: Action<{ targetId: string; quantity: number }> = ({ state, actions }, { targetId, quantity }) => {
...
export const setNodeMachinesCount: Action<{ nodeId: string; count: number | null }> = ({ state, actions }, { nodeId, count }) => {
