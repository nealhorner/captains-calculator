...
export const makeRecipe = (id: string, machineId: string, inputs: RecipeProduct[], outputs: RecipeProduct[]): Recipe => ({
    id: id as Recipe['id'],
    name: id,
...
export const buildTestWorld = () => {
    ...
    const smeltRecipe = makeRecipe('smelt_steel', 'smelter', [], [{ id: moltenSteel.id, name: moltenSteel.name, quantity: 12 }])
    const castRecipe = makeRecipe('cast_steel', 'caster', [{ id: moltenSteel.id, name: moltenSteel.name, quantity: 12 }], [{ id: steel.id, name: steel.name, quantity: 12 }])
    ...
    const smelterNode = makeNodeFor({ recipe: smeltRecipe, machine: smelter, category: metalCategory })
