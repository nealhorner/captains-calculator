...
/** What is written to localStorage and to an exported file. See `toSerialized`. */
export type SerializedProductionNode = {
  id: string;
  recipeId: RecipeId;
  /** Null when the count is derived from demand rather than set by hand. */
  pinnedMachinesCount: number | null;
  /** Which nodes supply each input product, by product id. */
  imports: { [productId: string]: string[] };
};
...
/**
 * Keeps the id counter ahead of anything restored from a file or localStorage,
 * so a node created later can never collide with a node loaded earlier.
 */
export const reserveNodeId = (id: string): void => {
  let suffix = Number(id.slice(id.lastIndexOf('_') + 1));
  if (Number.isFinite(suffix) && suffix > nodeSequence) nodeSequence = suffix;
};
...
class ProductionNode {
  ...
  constructor({
    ...
    let inputProducts = inputs.reduce(
      (items, item) => ({
        ...items,
        [item.id]: {
          ...item,
    ...
    let outputProducts = outputs.reduce(
      (items, item) => ({
        ...items,
        [item.id]: {
          ...item,
  ...
  removeImportLink(productId: string, sourceNodeId: string): void {
    ...
    this.inputs[productId].imports = this.inputs[productId].imports.filter(
      (i) => i.source !== sourceNodeId,
    );
  }
  ...
  removeExportLink(productId: string, targetNodeId: string): void {
    ...
    this.outputs[productId].exports = this.outputs[productId].exports.filter(
  ...
  /**
   * The persistable shape of this node.
   *
   * Only what the user chose is stored — the recipe, any hand-pinned building
   * count, and which nodes supply which input. Rates, imported/exported
   * amounts and deficits are all derived, so they are recomputed by the solver
   * on load rather than persisted and risking a stale or inconsistent file.
   */
  toSerialized(): SerializedProductionNode {
    let imports: SerializedProductionNode['imports'] = {};
    Object.values(this.inputs).forEach((input) => {
      let sources = input.imports.map((link) => link.source);
      if (sources.length) imports[input.id] = sources;
    });
    return {
      id: this.id,
